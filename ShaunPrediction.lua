...
function ShaunPrediction:GetEnemyHeroes(myHeroPos, range)
    local enemyHeroes = {}
...
function ShaunPrediction:checkCollision(myHeroPos, predictedPosition, ability)
        ...
        if abilityType == "linear" then
            ...
            for i = 1, steps do
                ...
                if collision["Hero"] then
                    local enemyHeroes = self:GetEnemyHeroes(myHeroPos, stepSize)
...
function ShaunPrediction:calculatePrediction(target, ability, source)
    ...
    if ability.type == "linear" then
        if self:checkCollision(myHeroPos, predictedPosition, ability) then
            return nil
        end
