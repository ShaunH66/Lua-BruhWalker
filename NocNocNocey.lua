...
local function GetAllyCountCicular(range, p1)
    ...
    Range = range * range
        if not unit.is_enemy and GetDistanceSqr2(p1, unit.origin) < Range and IsValid(unit) then
        count = count + 1
        end
    end
    return count
...
noc_combo_r_set_key = menu:add_keybinder("Semi Manual [R] Key", noc_category, 90)
...
local Q = { range = 1200, delay = .25, radius = 60, speed = 1600 }
...
local function Harass()
	...
	if menu:get_value(noc_haras_use_q) == 1 and GrabHarassMana then
		if ml.IsValid(qtarget) and IsKillable(qtarget) then
			if myHero:distance_to(qtarget.origin) <= Q.range then
				if ml.Ready(SLOT_Q) then
					CastQ(qtarget)
...
local function ManualR()
  ...
  if game:is_key_down(menu:get_value(noc_combo_r_set_key)) and ml.Ready(SLOT_R) then
    if myHero:distance_to(target.origin) <= rRange[spellbook:get_spell_slot(SLOT_R).level] then
			if ml.IsValid(target) and IsKillable(target) then
				CastR(target)
			end
